...
`@smai-kit/cmdsift-win32-x64` 是 cmdsift 的 `win32-x64` 平台二进制包，对应 Rust 编译目标 `x86_64-pc-windows-msvc`。
...
- 当前系统为 `win32-x64` 时，npm 下载并安装本包到 `node_modules/@smai-kit/cmdsift-win32-x64/`
- 当前系统为 `linux-x64` 时，npm 自动跳过本包，转而安装 `@smai-kit/cmdsift-linux-x64`
- 其他不匹配的平台同样被跳过
...
在 Windows x64 环境下，`process.platform` 为 `win32`，`process.arch` 为 `x64`，拼接出的包名为 `@smai-kit/cmdsift-win32-x64`，最终解析出的路径类似于：

```
node_modules/@smai-kit/cmdsift-win32-x64/bin/cmdsift.exe
```
...
该脚本会从 cmdsift 的 GitHub Release 页面下载 `x86_64-pc-windows-msvc` 平台的二进制压缩包，经 SHA256 校验后解压到 `packages/cmdsift-win32-x64/bin/cmdsift.exe`。
...
```
@smai-kit/cmdsift-win32-x64/
├── package.json
├── README.md
├── LICENSE
└── bin/
    └── cmdsift.exe    ← 预编译的 Windows x64 二进制文件（MSVC）
```
...
| 二进制文件名 | `cmdsift.exe` |
| 工具链 | MSVC |
| 兼容性 | 适用于 Windows 10 及以上版本的 x64 系统 |
